...
# def generate_feature_map(feature_fp, original_height=1002, original_width=1355, new_height=1002, new_width=1355):
#     # Read the CSV file
...
def generate_feature_map(feature_fp, original_height=1002, original_width=1355, new_height=1002, new_width=1355):
    ...
    df = pd.read_csv(feature_fp, sep=" ", header=0, names=['row', 'column', 'depth'])
...
def evaluate(dataset_path, depth_predictor, nsamples, sml_model_path):
    ...
    with open(f"{dataset_path}/image_depth_paths.txt", newline='') as txtfile:
